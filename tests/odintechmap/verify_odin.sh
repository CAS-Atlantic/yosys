...
function sim() {
	...
	yosys_synthesis_script_file_name="synthesis.ys"
	yosys_simulation_script_file_name="simulation.ys"
		...
		do
			...
			then
				...
				then
					...
					_echo_args "${yosys_synthesis_command};" \
						> "${DIR}/${yosys_synthesis_script_file_name}"
				...
				then
					...
					yosys_simulation_command="tee -a ${DIR}/${simulation_log_file_name} odinsim
										${_simulation_params}
										${arch_cmd}
										-b ${sim_blif_file}
										-sim_dir ${DIR}"
					...
					simulation_command="${YOSYS_EXEC}
											...
											${DIR}/${yosys_simulation_script_file_name}"
					...
					then
						yosys_simulation_command="${yosys_simulation_command} -t ${input_vector_file} -T ${output_vector_file}"
						simulation_wrapper_file_name="${simulation_wrapper_predefined_io_file_name}"
					elif [ "_${_generate_bench}" == "_off" ] \
					&& [ -f "${input_vector_file}" ]
					then
						yosys_simulation_command="${yosys_simulation_command} -t ${input_vector_file}"
					...
					_echo_args "${yosys_simulation_command};" \
						> "${DIR}/${yosys_simulation_script_file_name}"
